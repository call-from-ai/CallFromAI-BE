...
/**
 * 방의 채팅 사진을 DB와 S3에서 함께 정리하는 컴포넌트(캐릭터 하드 삭제 시 사용).
...
public class ChatPhotoCleaner {
    ...
    /** 반드시 트랜잭션 안에서 호출한다(afterCommit이 동작해야 하므로). */
    ...
    private void deleteFromS3(List<String> photoUrls) {
        for (String url : photoUrls) {
            try {
                ...
                log.warn("캐릭터 삭제 시 S3 사진 삭제 실패(고아 객체가 남을 수 있음). url={}", url, e);
            }
        }
    }
}
