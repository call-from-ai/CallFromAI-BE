...
public class CallVoiceResolver {
    ...
    /**
     * 캐릭터·프리셋을 못 읽었을 때의 최후 성별. 이 지점에선 캐릭터 성별조차 모르는 상태라
    ...
    public TTSVoice resolve(Long characterId) {
        try {
            ...
            if (character == null) {
                ...
                return TTSVoice.defaultFor(UNKNOWN_GENDER_FALLBACK);
            }
            ...
            log.error("[Call] 통화 음성 조회 실패 → 기본 목소리 사용(통화는 유지). characterId={}", characterId, e);
            return TTSVoice.defaultFor(UNKNOWN_GENDER_FALLBACK);
        }
    }
}
