...
public class RelationshipController {
    ...
    @Operation(
            summary = "AI 대화 요약 조회",
            description = "해당 관계의 사용자와 AI 캐릭터가 나눈 대화를 200자 이내로 요약한다.")
    @GetMapping("/{relationshipId}/chat-summary")
    public ApiResponse<ChatSummaryResponse> getChatSummary(
            @AuthenticationPrincipal Long memberId,
            @PathVariable Long relationshipId) {
        return ApiResponse.onSuccess(chatSummaryService.getSummary(memberId, relationshipId));
    }
}
